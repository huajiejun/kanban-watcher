...
**Goal:** 从 vibe-kanban 本地 SQLite 和 JSONL 会话日志中提取 AI Token 使用量，按天聚合后存入远程 MariaDB

**Architecture:**
- 读取 `~/Library/Application Support/ai.bloop.vibe-kanban/` 下的 SQLite 和 JSONL 文件
- 解析 `codex/event/token_count` 事件提取 token 用量增量
- 关联 session 元数据获取 executor 类型
- 按 (天, executor) 聚合后 upsert 到 MariaDB `kw_token_usage_daily` 表
- 复用现有 `database` 配置，新增 `token_stats` 配置项

**注意：** 由于 token_count 事件仅在 session 结束时记录，按小时细分不准确，故改为按天聚合。
...
│   ├── sqlite.go          # 从 SQLite 读取 session 元数据
│   ├── distribute.go       # 按 session 时长分布 token 增量
│   └── collector.go       # 定时任务 collector
internal/config/config.go  # 新增 TokenStatsConfig 字段
internal/store/models.go   # 新增 TokenUsageDaily 模型
```
...
### Task 2: 添加 TokenUsageDaily 模型

**Files:**
- Modify: `internal/store/models.go`

- [ ] **Step 1: 添加 TokenUsageDaily 模型**
...
git commit -m "feat: 添加 TokenUsageDaily 模型"
```
...
- [ ] **Step 1: 在 InitSchema 中添加 kw_token_usage_daily 表创建**
...
- [ ] **Step 2: 添加 UpsertTokenUsageDaily 方法**
...
git commit -m "feat: 添加 UpsertTokenUsageDaily 方法"
```
...
func (c *Collector) collect() {
	...
	// 从 vibe-kanban SQLite 数据库读取 session 元数据

	// 按 (天, executor) 聚合
	aggregated := aggregateByDay(sessions)
...
func aggregateByDay(sessions []SessionToken) []*AggregatedUsage {
	// 按 (day, executor) 聚合
	type key struct {
		day      time.Time
		executor string
	...
	for _, s := range sessions {
		k := key{day: day, executor: s.Executor}
		if a, ok := agg[k]; ok {
...
4. 验证 MariaDB 数据：
```sql
SELECT * FROM kw_token_usage_daily ORDER BY stat_date DESC LIMIT 10;
```

...
export class CreatePRDialog extends LitElement {
  ...
  private async loadInitialData() {
    ...
    try {
      ...
      console.log("[CreatePRDialog] Initial repoId:", effectiveRepoId);

      // 如果没有传入 repoId，尝试从 workspace 获取
      if (!effectiveRepoId && this.workspaceId) {
      ...
      // 加载分支列表
      if (effectiveRepoId) {
        console.log("[CreatePRDialog] Fetching branches for repo:", effectiveRepoId);
